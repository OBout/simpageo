...
geo.currentPosition = function(){
	db("starting currentPosition");
	 if (geo.hasgeo()) {
	    navigator.geolocation.getCurrentPosition(function(position) {
	      startPos = position;
	      var startLat = startPos.coords.latitude;
	      var startLon = startPos.coords.longitude;
	      db("startlatcur " + startLat);
	      db("startloncur " + startLon);
	      var startLatLon = new google.maps.LatLng(startLat, startLon);
			mapOptions = {
			  center: startLatLon,
			  zoom: 15
			};
			var map = new google.maps.Map(document.getElementById("geo_container_googlemaps"),
			    mapOptions);
